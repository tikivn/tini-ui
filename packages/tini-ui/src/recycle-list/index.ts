  ...
  methods: {
    ...
    _scrollViewDidScrollThrottle(e, force) {
      if (!this.onScrollFuncTh) {
        this.onScrollFuncTh = throttle(this._scrollViewDidScroll, 800, {
          leading: false,
          trailing: true,
        });
      }
    ...
    _calcViewportIndexes(left, top, cb) {
      const that = this;
      const { beginIndex, endIndex, minTop, afterHeight, maxTop } = that.__calcViewportIndexes(
        left,
        top,
      );
      if (cb) {
    ...
    _recycleInnerBatchDataChanged(cb) {
      //batch page setDate and innerBeforeHeight
    ...
    scrollTo(newVal) {
      ...
      if (Math.abs(newVal - this._lastScrollTop) < this._pos.height) {
        this.setData({
          ////两次设置的scrollTop一样的情况下不会滚动，scroll-view滚动的时候不会主动设置innerScrollTop属性
    ...
    scrollToIndex(newVal) {
      ...
      if (Math.abs(calScrollTop - this._lastScrollTop) < this._pos.height) {
        this.setData({
          innerScrollTop:
            this.data.innerScrollTop == calScrollTop ? calScrollTop + 1 : calScrollTop,
        });
        return newVal;
      }
