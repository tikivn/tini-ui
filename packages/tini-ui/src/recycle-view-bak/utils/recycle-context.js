...
function RecycleContext({ id, dataKey, page, itemSize, useInPage, placeholderClass, root }) {
...
RecycleContext.prototype._recalculateSizeByProp = function (list, cb) {
  ...
  if (calcNewItems.length) {
    ...
    this.page.setData(obj, () => {
      // my.createSelectorQuery().select(itemSize.componentClass).boundingClientRect(rects => {
      //   compSize = rects;
...
RecycleContext.prototype._recalculateSize = function (list) {
  ...
  const obj = {
    ...
    totalHeight: sizeArray.length
      ? sizeArray[sizeArray.length - 1].beforeHeight + sizeArray[sizeArray.length - 1].height
      : 0,
  };
